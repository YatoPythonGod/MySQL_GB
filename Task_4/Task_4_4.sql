CREATE DATABASE IF NOT EXISTS task_4_4;
...
SELECT u.id, CONCAT(u.firstname, " ", u.lastname) AS "fullname", p.hometown, m.filename as "profile picture"
FROM users u
JOIN profiles p
ON u.id = p.user_id
LEFT JOIN media m
...
JOIN profiles p
ON u.id = p.user_id
LEFT JOIN (SELECT * FROM media WHERE id IN (SELECT photo_id FROM profiles)) m
ON p.photo_id = m.id;
...
FROM media m
WHERE media_type_id = (SELECT id FROM media_types WHERE name_type = "Photo") AND m.user_id = (SELECT id FROM users WHERE email = "arlo50@example.org");

-- (используя вложенные запросы)
-- !!! Как решить задачу без подзапросов (JOIN), буду признателен если напишите в комментарииALTER
...
-- (используя UNION)
-- не совсем понимаю зачем здесь использовать UNION... для примера добавил статус
